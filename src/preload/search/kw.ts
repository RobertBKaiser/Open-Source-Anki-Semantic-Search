...
function scoreUnigram(tok: Str): number {
  ...
  if (/^[A-Z][a-z]+$/.test(tok) && tok.length >= 7) score += 1.8
  // Uppercase plural acronyms (ARBs) — singularized later to ARB
  if (/^[A-Z]{3,7}s$/.test(tok)) score += 2.6
  // Proper hyphenated eponyms like Cheyne-Stokes
  if (/^[A-Z][\p{Ll}A-Za-z]+(?:-[A-Z][\p{Ll}A-Za-z]+)+$/u.test(tok)) score += 1.5
  if (tok.length >= 8) score += 0.4
  if (!hasHyphen && /(ing|ed)$/.test(tl)) score -= 0.6
  return score
}
...
export function extractKeywords(text: Str, topK = 8): Str[] {
  ...
  for (let i = 0; i < tokens.length - 2; i++) {
    ...
    if (tl2 === 'insensitivity' && tl3 === 'syndrome') {
      s = 6.0
      if (isAdjLike(t1) || isNounish(t1)) s += 0.5
    } else {
      const heads = new Set(['syndrome','disease','deficiency','cancer','alopecia'])
      if (heads.has(tl3) && (isAdjLike(t1) || isNounish(t1)) && (isAdjLike(t2) || isNounish(t2))) s = 4.2
    }
  ...
  for (let i = 0; i < lower.length; i++) {
    ...
    if (m >= lower.length) continue
    const b = tokens[m]!
    const accept = (t: string) => isNounish(t) || t.length >= 5
    if (accept(a)) uni.push([a, Math.max(2.6, scoreUnigram(a) + 1.0)])
    if (accept(b)) uni.push([b, Math.max(2.6, scoreUnigram(b) + 1.0)])
  }
  ...
  const keptTris: Str[] = []
  const keptBis: Str[] = []
  for (const [p] of tri) if (!keptTris.includes(p)) keptTris.push(p)
  for (const [p] of bi) {
    const lower = p.toLowerCase()
    const inTri = keptTris.some((tp) => tp.toLowerCase().includes(lower))
    if (!inTri) keptBis.push(p)
  }
  ...
  const merged: Array<[Str, number]> = [
    ...keptTris.map((p, i) => [p, 10 - i] as [Str, number]),
    ...keptBis.map((p, i) => [p, 5 - i] as [Str, number]),
    ...filteredUni
  ]
  ...
  const normalize = (term: string): string => {
    if (/^[A-Z]{2,7}s$/.test(term)) return term.slice(0, -1)
    return term
  }
